...
openssl req -x509 -new -nodes -key local-ca.key -sha256 -days 1 -out local-ca.pem \
    -subj '/C=XX/ST=X/L=X/O=prism/CN=local-ca' \
    -addext "keyUsage=critical,digitalSignature,keyCertSign"

# Key and csr for the server
openssl genrsa -out testserver.key 2048
openssl req -new -key testserver.key -out testserver.csr \
    -subj '/C=XX/ST=X/L=X/O=prism/CN=localhost'
