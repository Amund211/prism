...
def make_session_payload(
    ...
    *,
    ...
    return {
        ...
        "refreshUntilInSeconds": refresh_until_in,
...
def test_login_stores_session_and_returns_view() -> None:
    ...
    # Fix the monotonic clock at 1000s so we can predict the deadline.
    client, _ = make_client_with_adapter(
        [FakeResponse(200, payload)], monotonic_clock=lambda: 1000.0
    )

    session = client.anonymous_login()
    ...
    assert (
        session.refresh_at_monotonic == 1000.0 + 3300
    ), "deadline = monotonic_now + refreshInSeconds"
...
def test_login_rejects_non_int_refresh_in_seconds() -> None:
    bad = make_session_payload()
    bad["refreshInSeconds"] = "not-a-number"
    client, _ = make_client_with_adapter([FakeResponse(200, bad)])
    with pytest.raises(FlashlightAuthError):
        client.anonymous_login()


def test_login_rejects_negative_refresh_in_seconds() -> None:
    bad = make_session_payload()
    bad["refreshInSeconds"] = -1
    client, _ = make_client_with_adapter([FakeResponse(200, bad)])
    with pytest.raises(FlashlightAuthError):
        client.anonymous_login()


def test_login_rejects_non_string_tier() -> None:
    bad = make_session_payload()
    bad["tier"] = 42
    client, _ = make_client_with_adapter([FakeResponse(200, bad)])
    with pytest.raises(FlashlightAuthError):
...
def test_login_rejects_non_bool_can_refresh() -> None:
    bad = make_session_payload()
    bad["canRefresh"] = "yes"
    client, _ = make_client_with_adapter([FakeResponse(200, bad)])
    with pytest.raises(FlashlightAuthError):
...
def test_login_carries_through_can_refresh_false_signal() -> None:
    """A response saying `canRefresh: false` lands on the parsed session as-is.

    The proactive timer reads this to decide refresh vs. re-login when it
    fires; the wire signal must round-trip through parsing untouched.
    """
    payload = make_session_payload(can_refresh=False)
